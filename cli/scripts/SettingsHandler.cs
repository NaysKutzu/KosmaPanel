    ...
    public class SettingsHandler
    {
        ...
        public void Setup() {
            if (fm.ConfigExists() == true) { 
                Program.logger.Log(LogType.Info,"Hi, and welcome to the automated installer for KosmaPanel.");
                ...
                try {
                    ...
                    {
                        connection.Open();
                        ExecuteSQLScript(connection, "INSERT INTO `kosma_panel_settings` (`name`, `logo`, `PterodactylURL`, `PterodactylAPIKey`) VALUES ('"+name+"', '"+logo+"', '"+panelkey+"','"+panelkey+"');");
                        connection.Close();
                    }
                } catch (Exception ex) {
